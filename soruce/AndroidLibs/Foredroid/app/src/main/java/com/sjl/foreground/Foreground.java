...
public class Foreground implements Application.ActivityLifecycleCallbacks {
            ...
            for (Iterator<WeakReference<Listener>> it = listeners.iterator(); it.hasNext(); ) {
                try {
                    ...
                    if (l != null && listener == l) {
                        Log.d(TAG, "remove," + listener.getClass().getSimpleName() + "@" + listener.hashCode() + ",dest " + l.getClass().getSimpleName() + "@" + l.hashCode());
                        break;
                    } else {
                        wr = null;
    ...
    public void onActivityStarted(Activity activity) {
        ...
        // remove any scheduled checks since we're starting another activity we're definitely not going background
        if (check != null) {
    ...
    public void onActivityPaused(Activity activity) {
        // if we're changing configurations we aren't going background so no need to schedule the check
